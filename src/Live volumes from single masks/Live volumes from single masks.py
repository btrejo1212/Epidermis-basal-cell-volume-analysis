...
# Example:
# CUSTOM_REPO_DIR = Path(r"C:\Users\brand\OneDrive\Desktop\Epidermis-basal-cell-volume-analysis")
CUSTOM_REPO_DIR = None

# Default input/output locations inside the repo
DEFAULT_INPUT_SUBDIR = Path("Data") / "Live volumes from single masks"
DEFAULT_OUTPUT_SUBDIR = Path("results") / "Live volumes from single masks"

# Optional override:
# Leave as None to use the default input/output folders above.
# Example:
# CUSTOM_INPUT_DIR = Path(r"D:\my_data\live_single_masks")
# CUSTOM_OUTPUT_DIR = Path(r"D:\my_outputs\live_single_masks_results")
...
def _z_index(p: Path) -> Optional[int]:
    """Extract Z index from filename, or return None if absent."""
    match = _RX_ZIDX.search(p.stem)
...
def _find_phase_dir(cell_dir: Path, phase: str) -> Optional[Path]:
    """
...
def _extract_phase_T(phase_dir: Optional[Path], is_birth: bool) -> Optional[int]:
    """
    ...
    for p in phase_dir.iterdir():
        if p.is_file():
            match = _RX_T_ANY.search(p.stem)
            if match:
                ts.add(int(match.group(1)))
...
def _find_split_dir(cell_dir: Path) -> Optional[Path]:
    """
...
def _collect_masks_zordered(zs_dir: Path) -> Tuple[List[int], List[np.ndarray]]:
    """
...
def _resolve_directories() -> Tuple[Path, Path, Path]:
    """
    Resolve input and output directories.

    Priority:
    1) CUSTOM_REPO_DIR if provided
    2) Infer from script location if running as a .py file
    3) Fall back to current working directory if running interactively
    """
    if CUSTOM_REPO_DIR is not None:
        repo_dir = Path(CUSTOM_REPO_DIR).resolve()
    else:
        try:
            # Assumes script lives in something like:
            # repo/src/<project_name>/script.py
            repo_dir = Path(__file__).resolve().parents[2]
        except NameError:
            # Notebook / interactive fallback
            repo_dir = Path.cwd().resolve()

    root_dir = Path(CUSTOM_INPUT_DIR).resolve() if CUSTOM_INPUT_DIR is not None else repo_dir / DEFAULT_INPUT_SUBDIR
    results_dir = Path(CUSTOM_OUTPUT_DIR).resolve() if CUSTOM_OUTPUT_DIR is not None else repo_dir / DEFAULT_OUTPUT_SUBDIR
...
def main():
    ...
    if not root_dir.exists():
        raise FileNotFoundError(
            ...
            f"Either:\n"
            f"1) place the dataset in the expected repo location, or\n"
    ...
    for cdir in cell_dirs:
        ...
        for tdir in t_dirs:
            match = re.search(r"[tT]\s*(\d+)", tdir.name)
            if match:
    ...
    out_xlsx = results_dir / "Cell_Frustum_Volumes_Timecourse_um3.xlsx"

    with pd.ExcelWriter(out_xlsx, engine="xlsxwriter") as writer:
        sheet_name = "FrustumVolumes_um3"
        ...
        worksheet.write(3, 0, "Timepoint_T")

        for j, meta in enumerate(cells_meta):
            col = 1 + j
            worksheet.write(0, col, f"{meta['name']} (Volume, um^3)")
