"""
Fixed membrane and nucleus volume analysis from Cellpose masks.

What this script does
---------------------
This script scans a dataset of fixed cells organized by embryo and position,
reads membrane and nucleus mask stacks, and computes per-cell shape metrics.

For each cell it computes, separately for membrane and nucleus:
- Frustum volume (um^3)
...
It also computes embryo-level median normalization for selected metrics and
writes one Excel workbook per position.
...
└── src/
    └── Fixed volumes of membrane and nucleus/
        └── fixed_membrane_nucleus_volumes.py
...
By default, this script uses:
    Data/Fixed volumes of membrane and nucleus

It writes results to:
    results/Fixed volumes of membrane and nucleus/<Embryo>/<Position>/
    FrustumVolumes_Membrane_Nucleus_byCell.xlsx

Optional overrides can be set in the USER SETTINGS section below.
...
Notes
-----
- If SciPy is available, the script:
  - keeps only the largest connected component in multi-component masks
  - uses convex hull distances for width estimation
- If SciPy is not available, it falls back to:
  - using the full mask
  - bounding-box diagonal for width estimation
"""
